-- ============================================================
-- WakaWay Edge Function: AI Report Verification
-- This Supabase Edge Function acts as the "Traffic Warden"
-- It validates user reports using Gemini AI before approval
-- ============================================================

-- Deploy this function using the Supabase CLI:
-- 1. supabase functions new verify-report
-- 2. Copy the TypeScript code below to supabase/functions/verify-report/index.ts
-- 3. Set secrets: supabase secrets set GEMINI_API_KEY=your_key_here
-- 4. Deploy: supabase functions deploy verify-report

/*
// ============ EDGE FUNCTION CODE (TypeScript) ============
// File: supabase/functions/verify-report/index.ts
...
-- ============================================================
-- Database Trigger: Auto-verify new contributions
-- This trigger calls the Edge Function when a new report is created
-- ============================================================

-- Create a function to call the Edge Function
CREATE OR REPLACE FUNCTION trigger_verify_contribution()
RETURNS TRIGGER AS $$
DECLARE
  edge_function_url TEXT;
  response JSONB;
BEGIN
  -- Get your Edge Function URL (set this as a secret or config)
  ...
  IF NEW.status = 'pending' THEN
    ...
    PERFORM net.http_post(
      url := edge_function_url || '/verify-report',
      body := json_build_object('contribution_id', NEW.id)::text,
      headers := '{"Content-Type": "application/json"}'
    );
  END IF;
...
-- Create trigger (optional - can verify manually or on-demand instead)
-- DROP TRIGGER IF EXISTS verify_new_contribution ON contributions;
-- CREATE TRIGGER verify_new_contribution
--   AFTER INSERT ON contributions
--   FOR EACH ROW
--   EXECUTE FUNCTION trigger_verify_contribution();

-- ============================================================
-- Manual verification function (call from client)
-- ============================================================
CREATE OR REPLACE FUNCTION request_verification(contribution_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  SELECT * INTO contribution FROM contributions WHERE id = contribution_id;
...
--    supabase functions new verify-report
--    
-- 2. Copy the TypeScript code (above) to:
--    supabase/functions/verify-report/index.ts
--    
-- 3. Set your Gemini API key:
--    supabase secrets set GEMINI_API_KEY=your_api_key_here
--    
-- 4. Deploy the function:
--    supabase functions deploy verify-report
--    
-- 5. Enable pg_net extension (for async HTTP calls):
--    Go to Supabase Dashboard > Database > Extensions > Enable pg_net
--    
-- 6. Set the Edge Function URL in your database config:
--    ALTER DATABASE postgres SET app.edge_function_url = 'https://your-project.supabase.co/functions/v1';
--    
-- 7. (Optional) Enable the trigger for automatic verification
-- ============================================================
